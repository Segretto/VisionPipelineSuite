"""
SAM 3 Object Tracking Script

How to run:
    python track_objects.py --input_path <path_to_video_or_images> --output_path <path_to_save_results>

    Example:
    python track_objects.py --input_path /path/to/video.mp4 --output_path ./results

Description:
    This script uses the SAM 3 model to track objects in a video or a sequence of images.
    It prompts the user to enter a list of object names to track.
    For each object, it initializes the SAM 3 model, adds a text prompt to the first frame,
    propagates the mask throughout the video, and saves the results in COCO JSON format.
    Finally, it merges all individual object annotations into a single JSON file.
"""
...
# --- Section 2: Video Loading Utilities ---
...
# --- Section 3: Mask Conversion Utilities ---
# These functions handle converting binary masks to RLE (Run-Length Encoding)
# and extracting bounding boxes, which are required for COCO format.
...
# --- Section 4: SAM 3 Model Interaction ---
# This function handles the communication with the SAM 3 predictor to propagate
# the segmentation masks across all frames of the video.
...
# --- Section 5: Class Processing (Placeholder) ---
...
# --- Section 6: Data Export (COCO JSON) ---
# This function takes the raw outputs from SAM 3 and formats them into
# the standard COCO JSON format for object detection and segmentation.
def save_coco_json(outputs_per_frame, video_frames, class_name, class_id, output_path):
    ...
    for frame_idx in sorted(outputs_per_frame.keys()):
        ...
        for sam_obj_id, obj_data in frame_data.items():
            ...
            # NEW: Verifica se a máscara é um array NumPy antes de qualquer operação.
...
# --- Section 7: Result Merging ---
# Combines multiple individual JSON files (one per object) into a single
# comprehensive JSON file containing all tracked objects.
...
# Parses arguments, initializes the model, handles user input for object names,
# and orchestrates the tracking and saving process for each object.
def main():
